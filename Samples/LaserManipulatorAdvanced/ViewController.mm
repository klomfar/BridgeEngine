...
#import <OpenBE/Components/SpawnComponent.h>
#import <OpenBE/Components/BridgeControllerComponent.h>
#import <OpenBE/Components/BridgeControllerManipulationComponent.h>
#import <OpenBE/Components/InputBeamComponent.h>

#import <OpenBE/Components/InteractablePhysicsComponent.h>
#import <OpenBE/Components/InteractableSpawnComponent.h>
...
// Create our laser pointer
-(void) setupLaserComponents {
    GKEntity *entity = _bControllerComponent.entity;
    ...
    [beamComponent start];
    [_bControllerComponent.node addChildNode:beamComponent.node];
    ...
    [_controllerManipulationComponent start];
    [_controllerManipulationComponent setEnabled:[BEController sharedController].isConnected];
...
- (void)mixedRealitySetUpSceneKitWorlds:(BEMappedAreaStatus)mappedAreaStatus
{
    ...
    // Adjust for a lighter gravity, feels really heavy otherwise
    [Scene main].scene.physicsWorld.gravity = SCNVector3Make(0,4,0);
...
- (void)mixedRealityUpdateAtTime:(NSTimeInterval)time
{
    ...
    GLKMatrix4 cameraTransform = SCNMatrix4ToGLKMatrix4(_mixedReality.localDeviceNode.worldTransform);
    [BEController sharedController].cameraTransform = cameraTransform;

    if( !_experienceIsRunning ) {
        return;
    }
    
    /* Putting this on the main thread removes a
     edit while mutating crash when I remove an entity from [SceneManager main].entities.
     */
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        if ((BEControllerButtonSecondary & buttonsDown) == BEControllerButtonSecondary) {
            ...
            if (intersectionPoint.x < INTERSECTION_FAR_DISTANCE) {
                [self.spawnComponent spawnWithPosition:
                SCNVector3Make(intersectionPoint.x,
...
//    [_bControllerComponent handleMotionTransform:transform];
    [self.controllerManipulationComponent handleMotionTransform:transform];
}
...
- (void) updateReticleInputMode {
    ...
    [self.controllerManipulationComponent setEnabled:enableReticleInput];
}
